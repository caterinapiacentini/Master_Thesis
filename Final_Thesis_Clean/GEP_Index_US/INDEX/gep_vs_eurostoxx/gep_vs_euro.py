...
# -*- coding: utf-8 -*-
"""
gep_vs_stoxx600_robust_min2.py
GEP Robust min-2 index vs STOXX Europe 600 — three comparison plots + regressions
with US FF3 and GPR controls, full sample and subsamples.

Ticker: EXSA.DE = iShares STOXX Europe 600 UCITS ETF (EUR-denominated, Xetra)
Available from ~2004-04 on yfinance.

Alternative tickers:
  EXW1.DE  — iShares STOXX Europe 600 (older share class)
  MEUD.PA  — Amundi STOXX Europe 600 (Euronext Paris, from 2006)

NOTE: EUR-denominated index. FX effects (EUR/USD) are NOT hedged.
      If you want USD-denominated comparison, consider using EFA instead.
      For a purely EUR perspective this is fine as-is.
"""
import os
import pandas as pd
import numpy as np
...
STOXX_TICKER = "EXSA.DE"          # change to "EXW1.DE" or "MEUD.PA" if needed
STOXX_LABEL  = "STOXX Europe 600 (EXSA.DE)"
START_DATE   = "2004-04-01"       # EXSA.DE inception date
...
# ── Download STOXX Europe 600 ──────────────────────────────────────────────────
print(f"Downloading {STOXX_LABEL}...")

stoxx_mo_raw = yf.download(STOXX_TICKER, start=START_DATE, end="2025-12-31",
                           interval="1mo", auto_adjust=True, progress=False)
if stoxx_mo_raw.empty:
    raise ValueError(f"No data returned for {STOXX_TICKER}. "
                     "Try EXW1.DE or MEUD.PA as alternative tickers.")

stoxx_mo = stoxx_mo_raw[["Close"]].copy()
stoxx_mo.index = stoxx_mo.index.to_period("M").to_timestamp()
stoxx_mo.columns = ["stoxx"]
stoxx_mo = stoxx_mo.sort_index()
stoxx_mo["log_ret"] = np.log(stoxx_mo["stoxx"] / stoxx_mo["stoxx"].shift(1))
stoxx_mo = stoxx_mo.dropna()

stoxx_d_raw = yf.download(STOXX_TICKER, start=START_DATE, end="2025-12-31",
                          interval="1d", auto_adjust=True, progress=False)
stoxx_d = stoxx_d_raw[["Close"]].copy()
stoxx_d.index = pd.to_datetime(stoxx_d.index)
stoxx_d.columns = ["stoxx"]
stoxx_d = stoxx_d.sort_index()
stoxx_d["log_ret"] = np.log(stoxx_d["stoxx"] / stoxx_d["stoxx"].shift(1))
stoxx_d = stoxx_d.dropna()

print(f"  Monthly: {stoxx_mo.index[0].date()} → {stoxx_mo.index[-1].date()} "
      f"({len(stoxx_mo)} obs)")
print(f"  Daily:   {stoxx_d.index[0].date()} → {stoxx_d.index[-1].date()} "
      f"({len(stoxx_d)} obs)")

# ── Download Fama-French 3 Factors (US) ───────────────────────────────────────
print("Downloading Fama-French 3 Factors (US)...")
ff3_mo_raw = web.DataReader('F-F_Research_Data_Factors', 'famafrench',
                            start=START_DATE, end='2025-12-31')[0]
...
# ── Merge ──────────────────────────────────────────────────────────────────────
df_mo = monthly[["GEP_monthly"]].join(stoxx_mo[["stoxx", "log_ret"]], how="inner")
...
df_d = daily[["GEP_daily"]].join(stoxx_d[["log_ret"]], how="inner")
...
df_z["stoxx_z"] = (df_z["stoxx"]       - df_z["stoxx"].mean())       / df_z["stoxx"].std()
...
ax.plot(df_z.index, df_z["stoxx_z"], color="#E74C3C", linewidth=1.1,
        label=f"{STOXX_LABEL} (Z-score)", zorder=3)
ax.fill_between(df_z.index, df_z["stoxx_z"], alpha=0.08, color="#E74C3C")
...
ax.set_title(f"GEP Robust min-2 vs STOXX Europe 600 — Monthly Z-scores (2004–2025)",
             fontsize=13, pad=12)
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_stoxx600_zscore.png"), dpi=150, bbox_inches="tight")
plt.close()

# ══════════════════════════════════════════════════════════════════════════════
# PLOT 2 — Two-panel: GEP monthly level + STOXX monthly log returns
# ══════════════════════════════════════════════════════════════════════════════
...
ax1.set_title(f"GEP Robust min-2 vs STOXX Europe 600 Monthly Log Returns (2004–2025)",
              fontsize=13, pad=10)
...
ax2.bar(df_mo.index, df_mo["log_ret"], color=colors, width=20, alpha=0.75,
        label="STOXX Europe 600 log return")
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_stoxx600_logret.png"), dpi=150, bbox_inches="tight")
plt.close()

# ══════════════════════════════════════════════════════════════════════════════
# PLOT 3 — Two-panel: GEP daily level + STOXX daily log returns
# ══════════════════════════════════════════════════════════════════════════════
...
ax1.spines["bottom"].set_visible(False)
ax1.legend(fontsize=10, framealpha=0.7, loc="upper left")
ax1.set_title(f"GEP Robust min-2 vs STOXX Europe 600 Daily Log Returns (2004–2025)",
              fontsize=13, pad=10)
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_stoxx600_logret_daily.png"), dpi=150, bbox_inches="tight")
plt.close()
...
# ── Subsamples ─────────────────────────────────────────────────────────────────
SUBSAMPLES_MO = {
    "Full sample (2004–2025)"            : (None,      None),
    "Pre-GEP era (2004–2017)"            : ("2004-04", "2017-12"),
...
SUBSAMPLES_D = {
    "Full sample (2004–2025)"            : (None,           None),
    "Pre-GEP era (2004–2017)"            : ("2004-04-01",   "2017-12-31"),
...
# ── MONTHLY ────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
print("MONTHLY REGRESSIONS — GEP vs STOXX Europe 600 (HAC Newey-West, 4 lags)")
print("="*80)
...
# ── DAILY ──────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
print("DAILY REGRESSIONS — GEP vs STOXX Europe 600 (HAC Newey-West, 10 lags)")
print("="*80)
