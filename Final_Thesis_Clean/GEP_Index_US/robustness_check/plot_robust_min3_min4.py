...
# -*- coding: utf-8 -*-
"""
plot_robust_min3_min4.py

Plots the robust min-3 and min-4 GEP indices normalized to 100 over 1996-2025,
following Caldara & Iacoviello (2022) convention.

Produces (6 files):
  GEP_Monthly_Robust_min3_norm100.png
  GEP_Daily_Robust_min3_norm100.png
  GEP_Daily_2025_Zoom_min3_norm100.png
  GEP_Monthly_Robust_min4_norm100.png
  GEP_Daily_Robust_min4_norm100.png
  GEP_Daily_2025_Zoom_min4_norm100.png
"""
...
def plot_version(label, monthly_file, daily_file):
    """Produce the three standard plots for one min-N version."""

    monthly = pd.read_csv(os.path.join(BASE, monthly_file))
    monthly["month"] = pd.to_datetime(monthly["month"])

    daily = pd.read_csv(os.path.join(BASE, daily_file))
    ...
    monthly["gep_norm"] = monthly["GEP_monthly"] / daily_mean * 100
    ...
    for month_str, lbl in PEAKS.items():
        ...
        if not row.empty:
            ...
            ax.annotate(
                lbl, xy=(x, y), xytext=(0, 14),
                textcoords="offset points", fontsize=7.5, ha="center",
                arrowprops=dict(arrowstyle="-", color="gray", lw=0.7),
                color="#333333",
            )

    ax.set_title(f"GEP Monthly Index — Robust {label}, normalized to 100 (1996–2025)",
                 fontsize=13, pad=12)
    ax.set_xlabel("")
    ...
    plt.tight_layout()
    out = os.path.join(BASE, f"GEP_Monthly_Robust_{label}_norm100.png")
    plt.savefig(out, dpi=150, bbox_inches="tight")
    print(f"Saved: {out}")
    plt.close()

    # ── Plot 2: Daily dots + monthly line ─────────────────────────────────────
    raw_peaks = [(find_nearby_peak(daily_obs, d)[0],
                  find_nearby_peak(daily_obs, d)[1], lbl) for d, lbl in EVENTS]
    ...
    ax.set_title(f"Daily GEP Index — Robust {label}, normalized to 100 (1996–2025)",
                 fontsize=13, pad=10)
    ...
    plt.tight_layout()
    out = os.path.join(BASE, f"GEP_Daily_Robust_{label}_norm100.png")
    ...
    for i, (date_str, lbl) in enumerate(TARIFF_2025):
        ...
        ax.text(
            dt, y_fracs[i % len(y_fracs)], lbl,
            transform=ax.get_xaxis_transform(),
            fontsize=7.5, ha="center", va="top", color="#8B0000", rotation=90,
    ...
    ax.tick_params(axis="x", labelsize=9)
    ax.set_ylabel("GEP Index (avg = 100)", fontsize=10)
    ax.set_title(f"Daily GEP Index — Robust {label} — 2025 Zoom", fontsize=13, pad=12)
    ax.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.0f"))
    ...
    plt.tight_layout()
    out = os.path.join(BASE, f"GEP_Daily_2025_Zoom_{label}_norm100.png")
    plt.savefig(out, dpi=160, bbox_inches="tight")
    print(f"Saved: {out}")
    plt.close()
...
# ── Run both versions ──────────────────────────────────────────────────────────
plot_version("min3", "GEP_Monthly_Robust_min3.csv", "GEP_Daily_Robust_min3.csv")
plot_version("min4", "GEP_Monthly_Robust_min4.csv", "GEP_Daily_Robust_min4.csv")
