...
class GTM():
    ...
    def run(self, params, pos_seed, neg_seed, output_dir, topic_name=None): 
        ...
        # Dynamically create filename based on topic_name, or fallback to seed words + timestamp
        if topic_name:
        ...
        log_path = os.path.join(output_dir, f"log_{self.filename}.txt")
        ...
        topics_dict = self.GenWordCloud(X, output_dir)
        topics_dict = pd.DataFrame.from_dict(topics_dict, orient='index', columns=["weight"])

        csv_path = os.path.join(output_dir, f"{self.filename}.csv")
        topics_dict.to_csv(csv_path, encoding='utf-8', index=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--combine_grid', action='store_true',  help="Combine 8 wordclouds into a 2x4 grid")
    parser.add_argument('--topic_name',   type=str,             required=False, help="Name of the sub-topic")
    parser.add_argument('--output_dir',   type=str,             required=True,  help="Path to output directory")
    parser.add_argument('--model_path',   type=str,             required=False, help="Path to Word2Vec .pkl file")
    parser.add_argument('--pos_words',    nargs='+', type=str,  required=False, help="Positive seed words")
    parser.add_argument('--pos_weights',  nargs='+', type=float,required=False, help="Weights for positive seeds")
    parser.add_argument('--neg_words',    nargs='+', type=str,  required=False, default=[], help="Negative seed words")
    parser.add_argument('--neg_weights',  nargs='+', type=float,required=False, default=[], help="Weights for negative seeds")
    ...
    if args.combine_grid:
        ...
        image_files = [
            "topic_Sanctions.png",          "topic_Trade_War.png",
            "topic_Export_Controls.png",    "topic_Tariffs.png",
            "topic_Financial_Coercion.png", "topic_Retaliation.png",
            "topic_Protectionism.png",      "topic_Embargo.png"
        ]
        fig, axes = plt.subplots(2, 4, figsize=(32, 14))
        axes = axes.flatten()
        
        for idx, img_name in enumerate(image_files):
            img_path = os.path.join(args.output_dir, "WordClouds", img_name)
        ...
        plt.tight_layout()
        grid_path = os.path.join(args.output_dir, "Combined_Geoeconomic_Pressure_Grid_8.png")
        plt.savefig(grid_path, dpi=300, facecolor='w', edgecolor='w')
        plt.close(fig)
    ...
    if run:
        ...
        gtm.run(params, pos_seed, neg_seed, output_dir=args.output_dir, topic_name=args.topic_name)
