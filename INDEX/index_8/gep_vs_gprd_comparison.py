...
# -*- coding: utf-8 -*-
"""
gep_vs_gprd_comparison.py

Compares the GEP Daily Index (this thesis) against the daily GPR index
(Caldara & Iacoviello 2022) from 1996 onwards.

Scaling note:
  - GPRD is normalised so its 1985-2019 mean = 100 (dimensionless index)
  - GEP score is a raw weighted word frequency (~10^-4 scale)
  Both are z-scored before any visual comparison.

Outputs (saved to INDEX/index_8/):
  1. gep_vs_gprd_timeseries.png   — z-scored time series + rolling corr
  2. gep_vs_gprd_crosscorr.png    — cross-correlation at lags -60 … +60 days
  Console: Pearson correlation, Granger causality (GEP→GPR, GPR→GEP)
"""
...
GPR_PATH  = "/Users/catepiacentini/Desktop/data_gpr_daily_recent.xls"
GEP_PATH  = f"{BASE}/GEP_Daily_Index.csv"

# ----------------------------------------------------------------
# 1. Load GPR — keep GPRD from 1996, use the 'date' column
# ----------------------------------------------------------------
gpr = pd.read_excel(GPR_PATH, parse_dates=['date'])
gpr = gpr[['date', 'GPRD']].rename(columns={'date': 'date', 'GPRD': 'GPRD'})
gpr = gpr[gpr['date'] >= '1996-01-01'].copy()
gpr['date'] = pd.to_datetime(gpr['date']).dt.normalize()

# ----------------------------------------------------------------
# 2. Load GEP — keep only true trading days (n_articles > 0)
# ----------------------------------------------------------------
gep = pd.read_csv(GEP_PATH, parse_dates=['date'])
gep = gep[gep['date'] >= '1996-01-01'].copy()
gep = gep[gep['n_articles'] > 0][['date', 'score']].rename(columns={'score': 'GEP'})

# ----------------------------------------------------------------
# 3. Merge on date (inner join — only days both indices have data)
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# 6. Granger causality (maxlag = 5 trading days)
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
df['roll_corr'] = (
    df['GEP_z'].rolling(90, min_periods=60)
    .corr(df['GPRD_z'])
)
...
# Top panel: z-scored series
ax1.plot(df['date'], df['GEP_z'],  color='#378ADD', linewidth=0.7,
         alpha=0.85, label='GEP (this thesis)')
...
for date_str, label in key_events.items():
    ...
    if df['date'].min() <= xd <= df['date'].max():
        ...
        ax1.text(xd, ax1.get_ylim()[1] if ax1.get_ylim()[1] > 0 else 5,
                 label, rotation=90, fontsize=6.5, va='top',
                 color='#555555', ha='right')

ax1.axhline(0, color='black', linewidth=0.4, linestyle='-')
ax1.set_ylabel('Z-score', fontsize=10)
ax1.set_title(f'GEP vs GPR Daily Index (1996–2025)  |  r = {r:.4f}', fontsize=13, pad=10)
...
plt.tight_layout()
out1 = f"{BASE}/gep_vs_gprd_timeseries.png"
plt.savefig(out1, dpi=150, bbox_inches='tight')
print(f"\nSaved: {out1}")
plt.close()

# ----------------------------------------------------------------
# 9. Plot 2: Cross-correlation at lags -60 … +60 days
#    Positive lag k: GEP at t-k vs GPRD at t (GEP leads GPRD by k days)
#    Negative lag k: GPRD leads GEP
# ----------------------------------------------------------------
max_lag = 60
...
for lag in lags:
    if lag == 0:
        ...
        # GEP leads: correlate GEP[:-lag] with GPRD[lag:]
        xcorrs.append(df['GEP_z'].iloc[:-lag].corr(df['GPRD_z'].iloc[lag:]))
    else:
        # GPRD leads: correlate GEP[abs(lag):] with GPRD[:lag]
        xcorrs.append(df['GEP_z'].iloc[-lag:].corr(df['GPRD_z'].iloc[:lag]))
...
ax.bar(lags_arr, xcorrs_arr, width=1.0, color=np.where(lags_arr >= 0, '#378ADD', '#E05C2A'), alpha=0.7)
...
plt.tight_layout()
out2 = f"{BASE}/gep_vs_gprd_crosscorr.png"
