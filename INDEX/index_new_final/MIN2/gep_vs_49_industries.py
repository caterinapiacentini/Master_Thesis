...
# ── 2. Load GPR Controls ───────────────────────────────────────────────────────
# Daily GPR: col 'date' (dd/mm/yy), col 'GPRD'
...
# Monthly GPR: col 'month' (dd/mm/yy), col 'GPR'
...
ff3_m = ff3_m_raw.copy()
ff3_m.index = ff3_m.index.to_timestamp()

# ── 4. FF49 full name mapping ──────────────────────────────────────────────────
FF49_NAMES = {
...
# ── 6. Regression engines ──────────────────────────────────────────────────────
def run_regressions_daily(gep_series, ind_df, ff_df, gpr_ctrl):
    """Daily regression: controls = FF3 + GPR_daily"""
    results = []
    for industry in ind_df.columns:
        df = pd.concat([ind_df[industry], gep_series, ff_df, gpr_ctrl], axis=1).dropna()
        df.columns = ['RET', 'GEP', 'MktRF', 'SMB', 'HML', 'RF', 'GPR_daily']

        df['y']       = df['RET'] - df['RF']
        df['gep_pct'] = df['GEP'] * 100

        controls = ['MktRF', 'SMB', 'HML', 'GPR_daily']
...
def run_regressions_monthly(gep_series, ind_df, ff_df, gpr_ctrl):
    """Monthly regression: controls = FF3 + GPR_monthly"""
    results = []
    for industry in ind_df.columns:
        df = pd.concat([ind_df[industry], gep_series, ff_df, gpr_ctrl], axis=1).dropna()
        df.columns = ['RET', 'GEP', 'MktRF', 'SMB', 'HML', 'RF', 'GPR_monthly']

        df['y']       = df['RET'] - df['RF']
        df['gep_pct'] = df['GEP'] * 100

        controls = ['MktRF', 'SMB', 'HML', 'GPR_monthly']
...
# ── 7. Run regressions ─────────────────────────────────────────────────────────
res_d = run_regressions_daily(daily_gep["GEP_daily"], ind_d, ff3_d, gpr_daily)
res_m = run_regressions_monthly(monthly_gep["GEP_monthly"], ind_m, ff3_m, gpr_monthly)
...
print_results(res_d, "DAILY INDUSTRY PORTFOLIOS",
              "FF3 (MktRF, SMB, HML) + GPR daily")
print_results(res_m, "MONTHLY INDUSTRY PORTFOLIOS",
              "FF3 (MktRF, SMB, HML) + GPR monthly")

# ── Full coefficient tables: Daily (Contemp + Predictive) ─────────────────────
print(f"\n{'='*100}")
print(" FULL COEFFICIENTS: DAILY CONTEMPORANEOUS")
print(f"{'='*100}")

for industry in ind_d.columns:
    df = pd.concat([ind_d[industry], daily_gep["GEP_daily"], ff3_d, gpr_daily], axis=1).dropna()
    df.columns = ['RET', 'GEP', 'MktRF', 'SMB', 'HML', 'RF', 'GPR_daily']
    df['y']       = df['RET'] - df['RF']
    df['gep_pct'] = df['GEP'] * 100
    X = sm.add_constant(df[['gep_pct', 'MktRF', 'SMB', 'HML', 'GPR_daily']])
...
for industry in ind_d.columns:
    ...
    df['gep_lag'] = df['gep_pct'].shift(1)
    df = df.dropna()
    X = sm.add_constant(df[['gep_lag', 'MktRF', 'SMB', 'HML', 'GPR_daily']])
    m = sm.OLS(df['y'], X).fit(cov_type='HC3')
    print(f"\n--- {industry.strip()} ---")
    summary = pd.DataFrame({
        'Beta':  m.params,
...
def plot_exposure(df, beta_col, pval_col, title, filename):
    ...
    fig.text(0.5, 0.01,
             ...
             "Controls: FF3 (MktRF, SMB, HML) + GPR daily. SE: HC3.",
