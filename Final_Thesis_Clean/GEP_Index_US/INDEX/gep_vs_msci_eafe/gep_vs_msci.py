...
# -*- coding: utf-8 -*-
"""
gep_vs_efa_robust_min2.py
GEP Robust min-2 index vs MSCI EAFE (EFA) — three comparison plots + regressions
with US FF3 and GPR controls, full sample and subsamples.

EFA = iShares MSCI EAFE ETF — developed markets ex-US/Canada, USD-denominated.
Available from 2001-08 on yfinance.
"""
import os
import pandas as pd
import numpy as np
...
START_DATE  = "2001-08-01"   # EFA inception date
...
# ── Download EFA ───────────────────────────────────────────────────────────────
print("Downloading MSCI EAFE (EFA) data...")

efa_mo_raw = yf.download("EFA", start=START_DATE, end="2025-12-31",
                         interval="1mo", auto_adjust=True, progress=False)
efa_mo = efa_mo_raw[["Close"]].copy()
efa_mo.index = efa_mo.index.to_period("M").to_timestamp()
efa_mo.columns = ["efa"]
efa_mo = efa_mo.sort_index()
efa_mo["log_ret"] = np.log(efa_mo["efa"] / efa_mo["efa"].shift(1))
efa_mo = efa_mo.dropna()

efa_d_raw = yf.download("EFA", start=START_DATE, end="2025-12-31",
                        interval="1d", auto_adjust=True, progress=False)
efa_d = efa_d_raw[["Close"]].copy()
efa_d.index = pd.to_datetime(efa_d.index)
efa_d.columns = ["efa"]
efa_d = efa_d.sort_index()
efa_d["log_ret"] = np.log(efa_d["efa"] / efa_d["efa"].shift(1))
efa_d = efa_d.dropna()
...
# ── Merge ──────────────────────────────────────────────────────────────────────
df_mo = monthly[["GEP_monthly"]].join(efa_mo[["efa", "log_ret"]], how="inner")
...
df_d = daily[["GEP_daily"]].join(efa_d[["log_ret"]], how="inner")
...
df_z["efa_z"] = (df_z["efa"]         - df_z["efa"].mean())         / df_z["efa"].std()
...
ax.plot(df_z.index, df_z["efa_z"], color="#E74C3C", linewidth=1.1,
        label="MSCI EAFE / EFA (Z-score)", zorder=3)
ax.fill_between(df_z.index, df_z["efa_z"], alpha=0.08, color="#E74C3C")
...
ax.set_title("GEP Robust min-2 vs MSCI EAFE (EFA) — Monthly Z-scores (2001–2025)",
             fontsize=13, pad=12)
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_efa_zscore.png"), dpi=150, bbox_inches="tight")
plt.close()

# ══════════════════════════════════════════════════════════════════════════════
# PLOT 2 — Two-panel: GEP monthly level + EFA monthly log returns
# ══════════════════════════════════════════════════════════════════════════════
...
ax1.set_title("GEP Robust min-2 vs MSCI EAFE (EFA) Monthly Log Returns (2001–2025)",
              fontsize=13, pad=10)
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_efa_logret.png"), dpi=150, bbox_inches="tight")
plt.close()

# ══════════════════════════════════════════════════════════════════════════════
# PLOT 3 — Two-panel: GEP daily level + EFA daily log returns
# ══════════════════════════════════════════════════════════════════════════════
...
ax1.spines["bottom"].set_visible(False)
ax1.legend(fontsize=10, framealpha=0.7, loc="upper left")
ax1.set_title("GEP Robust min-2 vs MSCI EAFE (EFA) Daily Log Returns (2001–2025)",
              fontsize=13, pad=10)
...
plt.tight_layout()
plt.savefig(os.path.join(BASE, "gep_vs_efa_logret_daily.png"), dpi=150, bbox_inches="tight")
plt.close()
...
# ── Subsamples ─────────────────────────────────────────────────────────────────
SUBSAMPLES_MO = {
    "Full sample (2001–2025)"            : (None,      None),
    "Pre-GEP era (2001–2017)"            : ("2001-08", "2017-12"),
...
SUBSAMPLES_D = {
    "Full sample (2001–2025)"            : (None,           None),
    "Pre-GEP era (2001–2017)"            : ("2001-08-01",   "2017-12-31"),
...
# ── MONTHLY ────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
print("MONTHLY REGRESSIONS — GEP vs MSCI EAFE (HAC Newey-West, 4 lags)")
print("="*80)
...
# ── DAILY ──────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
print("DAILY REGRESSIONS — GEP vs MSCI EAFE (HAC Newey-West, 10 lags)")
print("="*80)
