...
# Country tag extraction from Reuters subjects:
...
def extract_meta(item: Dict, gz_path: Path) -> Dict:
    """
    Extract key identifiers/timestamps + Reuters classification for later mapping.
    """
...
def normalize_text(raw: str) -> str:
    ...
    # remove standard dateline + editorial credits
    x = RE_DATALINE.sub("", x)
    x = RE_CREDITS.sub("", x)

    # remove bracket tags and common boilerplate lines/phrases
    x = RE_BRACKETS.sub(" ", x)
...
def list_year_gz_files(year_dir: Path, pattern: str = "*.txt.gz") -> List[Path]:
    """
    Return sorted gz files inside year_dir. By default matches any *.txt.gz.
    """
...
def iter_docs_with_meta(
    ...
    """
    Iterate documents for a given year directory, optionally restricting to a subset of files.

    Parameters
    ----------
    file_offset : int
        Start index (0-based) in the sorted list of files.
    n_files : int
        Number of files to process from offset. 0 means "all remaining".
    file_glob : str
        Glob pattern for Reuters month files inside year_dir.
    """
...
def main():
    ...
    ap.add_argument("--file_offset", type=int, default=0,
                    help="Start index in sorted *.txt.gz list (0-based).")
    ap.add_argument("--n_files", type=int, default=0,
                    help="How many files to process from offset. 0 = all remaining.")
    ap.add_argument("--file_glob", type=str, default="*.txt.gz",
                    help="Glob for Reuters files inside year_dir (default: *.txt.gz).")
    ...
    # Ignore these when LEARNING phrases (but still keep them in final corpus)
    phrase_ignore_terms = {
